...
class PEVL_Vrd(nn.Module):
    ...
    def forward(self, image=None, text=None, mask_text=None, epoch=None, alpha=0, mode='itc', i=None, ):
        if mode == 'pretrain':
            with torch.no_grad():
            ...
            target_input_ids = text.input_ids.clone()
            ...
            with torch.no_grad():
                ...
                ##================= MLM ========================##
            input_ids = mask_text.input_ids.clone()
            target_input_ids = text.input_ids.clone()
            labels = target_input_ids.clone()

            probability_matrix = torch.full(labels.shape, self.mlm_probability)

            input_ids_, labels = self.rel_mask(input_ids, targets=labels, probability_matrix=probability_matrix)
            with torch.no_grad():
                logits_m = self.text_encoder_m(input_ids_,
                                               attention_mask=text.attention_mask,
                                               encoder_hidden_states=image_embeds_m,
    ...
    def rel_mask(self, input_ids, targets=None, masked_indices=None, probability_matrix=None, ):
        if masked_indices is None:
            masked_indices = torch.bernoulli(probability_matrix).bool()
        masked_indices[:] = False
        masked_indices[input_ids == 103] = True
