...
class PEVL_pretrain(nn.Module):
    def __init__(self,
        ...
        if init_deit:
            ...
            checkpoint = torch.load('/mnt/sfs_turbo/chenqianyu/ALBEF/deit_base_patch16_224-b5f2ef4d.pth', \
                                    map_location='cpu')
        ...
        # print(len(self.text_encoder.bert.embeddings))
        # exit()
        ...
        self.register_buffer("image_idx_queue", torch.full((1, self.queue_size), -100))
    ...
    def exp_decay(self, num):
        return np.exp(-self.exp_decay_ratio * num)

    def forward(self, image=None, text=None, alpha=0, itm_labels=None, mode='grounding',
                ):
        if mode == 'vcr':
            with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                # for index, yy in  enumerate(itm_label_):
                #     sim_targets[index,index] = yy
            ...
            input_ids_, labels = self.mask(input_ids, self.text_encoder.config.vocab_size, image.device, targets=labels, probability_matrix=probability_matrix)
            with torch.no_grad():
                logits_m = self.text_encoder_m(input_ids_,
                                               attention_mask=text.attention_mask,
                                               encoder_hidden_states=image_embeds_m,
    ...
    def mask(self, input_ids, vocab_size, device, targets=None, masked_indices=None, probability_matrix=None):
        ...
        # mask pos token
        masked_indices[(input_ids > self.min_pos) & (input_ids < self.max_pos)] = False
        pos_index = (input_ids.reshape(-1) >= self.min_pos) & (input_ids.reshape(-1) <= self.max_pos)
        source_shape = masked_indices.shape
        masked_indices = masked_indices.reshape(-1)

        if True in pos_index:
            pos_start_index = torch.where(input_ids.reshape(-1) == self.min_pos)[0]
